...
<%
    ...
    try {
        int i = sto.executeUpdate("UPDATE hospital_ordrs SET status='Approved' WHERE id='" + oid + "' ");
        if (i != 0) {
            ResultSet rs = st.executeQuery("SELECT * FROM hospital_ordrs where id='" + oid + "' ");
            if (rs.next()) {
                ...
                response.sendRedirect("View_hospitalorders.jsp?Order_Approved");
            } else {
                response.sendRedirect("View_hospitalorders.jsp?Failed");
            }
        }
    } catch (Exception ex) {
        ex.printStackTrace();
